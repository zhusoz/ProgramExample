        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(datasql) && connstr != null&&!string.IsNullOrEmpty(tablename))
                {
                    ...
                    db.Open();
                    await Logger.AddDBLog("登录数据库", LogLevel.Information, connstr.ConnectionString);
                    ...
                    for (int x = 1; x <= intcount; x++)
                    {
                        ...
                        await Logger.AddDBLog($"{tablename}表迁移", LogLevel.Information, null, connstr.ConnectionString);
                    }

                    db.Dispose();
                    db.Close();
                    await Logger.AddDBLog("登出数据库", LogLevel.Information, connstr.ConnectionString);
                }
            }
