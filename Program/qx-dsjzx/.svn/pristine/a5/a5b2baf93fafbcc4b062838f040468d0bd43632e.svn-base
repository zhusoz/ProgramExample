...
namespace DataSharing.Controllers
{
    ...
    public class HomeController : ControllerBase
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<AjaxResult<DataConverge>> GetDataConverge()
        {
            //3.数据汇聚
            //#查询指定数据库表数量
            ...
            var result = await _dataTableService.GetEntity<DataConverge>(countSql);
            return new AjaxResult<DataConverge>(result);
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<PageResult<List<DataMapCollectDto>>> Search(SearchQueryDto dto)
        {
            //预检
            ...
            {
                return new PageResult<List<DataMapCollectDto>>();
            }
            ...
            var result = await _dataTableService.GetEntityPageList<DataMapCollectDto>(itemSql, dto.PageIndex, dto.PageSize, total);

            await Logger.AddPlatformLog($"发起数据检索,关键字:{dto.KeyWord}", LogType.DataAccess);
            return new PageResult<List<DataMapCollectDto>>(result, total, total%dto.PageSize==0 ? total/dto.PageSize : total/dto.PageSize+1, dto.PageSize, dto.PageIndex);
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<AjaxResult<List<SearchResultGroupByDto>>> SearchRelation(SearchRelationQueryDto dto)
        {
            ...
            //1.左侧单位分组列表
            string leftAttrSql = null;
            if (string.IsNullOrEmpty(dto.KeyWord))
            {
                return new AjaxResult<List<SearchResultGroupByDto>>();
            }
            leftAttrSql = $"select attr.Id,attr.Attribution from `datamap` t1 left join `frequency` t2 on t1.Frequency = t2.ID left join `layered` t3 on t1.`layeredType` = t3.`id` left join `modified` t4 on t1.`modifierType` = t4.`id` join `attribution` attr on t1.`attribution` = attr.Id where t1.`Status`=1 and t1.`RackStatus`=0 and t1.`IsMapping` is null and t1.`Name` like '%{dto.KeyWord}%' group by attr.Id";
            List<SearchResultGroupByDto> result = await _dataTableService.GetEntityList<SearchResultGroupByDto>(leftAttrSql);
            result.ForEach(async e =>
            {
                //2.单位领域分类列表
                string attrLayeredCountSql = $"select attr.Id,attr.Attribution,t1.LayeredType,t3.`Name` Layered,count(1) Count from `datamap` t1 left join `frequency` t2 on t1.Frequency = t2.ID left join `layered` t3 on t1.`layeredType` = t3.`id` left join `modified` t4 on t1.`modifierType` = t4.`id` join `attribution` attr on t1.`attribution` = attr.Id where t1.`Status`=1 and t1.`RackStatus`=0 and t1.`IsMapping` is null and attr.Id={e.Id} and t1.`Name` like '%{dto.KeyWord}%' group by attr.Id,t1.LayeredType";
                e.Items=await _dataTableService.GetEntityList<SearchResultGroupByLayeredDto>(attrLayeredCountSql);
            });

            await Logger.AddPlatformLog($"{dto.KeyWord}相关数据检索", LogType.DataAccess);
            return new AjaxResult<List<SearchResultGroupByDto>>(result);
        }
