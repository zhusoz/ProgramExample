...
namespace DataSharing.Controllers
{
    ...
    public class RepairOrderController : ControllerBase
    {
        ...
        public async Task<AjaxResult<string>> AddRepairOrder(AddRepairOrderDto dto)
        {
            ...
            await _opiService.Add(new order_processinfo { Opinion = dto.Opinion, ProcessId = processid,Node = 0});
        ...
        {
            var sql =
                $" select pt.id, ro.number `no`,ro.title,ro.starttime submittime,ro.user username,ro.phone ,ro.priority,pd.dict_info priorityname, case when  TIMEdiff(  ro.EndTime,NOW())<0 and pt.status<2 then '已超时' else p.`Describe` end  process  from  ( select * from  public_task where TaskType=4) pt join repair_order ro on ro.id=pt.InfoRelation join (select * from  public_dict where dict_key='order_priority'  ) pd on pd.dict_value=ro.Priority  join process  p on p.id=pt.CurrentProcess where 1=1  {(dto.Status == 0 ? " " :dto.Status==1? $" and  pt.status={dto.Status} and  TIMEdiff(ro.EndTime,NOW())>0" :$" and ( pt.status={dto.Status} or   TIMEdiff( ro.EndTime,NOW())< 0  )")} ";
            var sqlwhere = "";
            if (dto.StartTime != null && dto.EndTime != null) sqlwhere +=  $" and timediff('{dto.EndTime.Value.ToString("yyyy/MM/dd HH:mm:ss")}',p.CreateTime)>0 and  timediff('{dto.StartTime.Value.ToString("yyyy/MM/dd HH:mm:ss")}',p.CreateTime)<0 " ;
            if (dto.Priority != null) sqlwhere += $" and ro.priority={dto.Priority} ";
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<AjaxResult<ReturnAllOrderDetailsDto>> GetAllOrderDetails(int taskid)
        {
            var sql =
            ...
            var result = await _dtService.GetEntity<ReturnAllOrderDetailsDto>(sql);
            ...
            return new AjaxResult<ReturnAllOrderDetailsDto>(result);
        }
        ...
        {
            var sql =
                $"select pt.id, ro.Number `no`,ro.`User` username,ro.Phone,ro.Department,ro.StartTime,ro.title,ro.field ,ro.type,ro.content ,t1.dict_info NodeName,op.node,op.Opinion from ( select * from  public_task where TaskType=4) pt join repair_order ro on ro.id=pt.InfoRelation  join process  p on p.id=pt.CurrentProcess left join order_processinfo op on op.ProcessId=p.Id left join (select * from public_dict where dict_key='order_node' )t1 on t1.dict_value=op.node where pt.id={taskid} ";
